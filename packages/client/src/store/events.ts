...
import { CreateEventDto } from '@omc/server/event/dto/create-event.dto'
import { FindAllEventsDto } from '@omc/server/event/dto/find-all-events.dto'
...
export const actions = actionTree(
  { state, getters, mutations },
  {
    ...
    async delete(
      { commit },
      { id, mode }: { id: number | string; mode: 'single' | 'future' | 'all' }
    ) {
      try {
        commit('setStatus', StateStatus.BUSY)

        await this.$axios.$delete(`/event/${id}/${mode}`)
