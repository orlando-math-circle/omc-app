...
export class EventRegistrationController {
  ...
  @UserAuth('volunteer-registration', 'create:own')
  @Post('/volunteer/:eventRegistrationId')
  swapVolunteers(
    @Param('eventRegistrationId') eventRegistrationId: number,
    @Body() { eventId, users }: CreateVolunteerRegistrationDto,
    @Usr() user: User,
  ) {
    return this.registrationService.volunteer(eventId, users, user, eventRegistrationId);
  }
  ...
  @UserAuth('event-registration', 'create:own')
  @Get('/status/swap/:eventId')
  getSwapRegistrations(@Param('eventId') eventId: number, @Usr() user: User) {
    return this.registrationService.getSwapRegistrations(eventId, user);
  }

  @AccountAuth()
  @Get('/status/:eventId')
  getStatus(@Param('eventId') eventId: number, @Acc() account: Account, @Usr() user: User) {
    return this.registrationService.getRegistrationStatus(eventId, account, user);
  }
