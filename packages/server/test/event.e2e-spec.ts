...
describe('Events', () => {
  ...
  describe('PATCH /event/:id/future', () => {
    ...
    it('should not reset a count in a recurrence split', async () => {
      ...
      const secondCreatedEvent = resp.body[1].id;
      const thirdCreatedEvent = resp.body[2].id;

      expect(typeof secondCreatedEvent).toBe('number');
      expect(typeof thirdCreatedEvent).toBe('number');
      ...
      const newEvents = await orm.em.find(Event, { description: 'Split' }, [
        'recurrence',
      ]);

      expect(newEvents).toHaveLength(3);
      expect(newEvents[0].recurrence).toBeDefined();
      expect(newEvents[0].recurrence?.rrule.includes('COUNT=3')).toBeDefined();
    });
  ...
  describe('PATCH /event/:id/all', () => {
    ...
    it('should reset an exception if dtstart > dtend > originalStart', async () => {
      const createDto: CreateEventDto = {
        name: 'Reset Exception Test',
        rrule: {
          freq: Frequency.DAILY,
          dtstart: new Date(Date.UTC(2020, 0)),
          until: new Date(Date.UTC(2020, 0, 10)),
        },
      };
